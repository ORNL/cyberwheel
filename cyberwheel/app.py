...
def main_page_layout():
    ...
    for path in os.scandir(basepath):
        if os.path.isdir(path.path) and any(".pickle" in g for g in os.listdir(path.path)):
            graph_list.append(path.name)
    #print(plugin_list)
    graph_dict = {"name": []}
    for graph in graph_list:      
        graph_dict["name"].append(f"[{graph}]({dash.get_relative_path(f'/graph/{graph}')})")
    df = pd.DataFrame(graph_dict, columns=list(graph_dict.keys()))
    return dash_table.DataTable(df.to_dict('records'), [{'id': x, 'name': x, 'presentation': 'markdown'} if x == 'name' else {'id': x, 'name': x} for x in df.columns], markdown_options={"link_target": '_self', "html": False})
...
def graph_page_layout(graph_name : str):
    ...
    return html.Div([
        ...
        dcc.Dropdown(id="input-episode-dropdown",options=sorted(num_episodes), placeholder="Select Episode", value=0),
        html.P("Choose Step: "),
        dcc.Slider(0, num_steps - 1, 1, value=0, id='input-step-slider', updatemode='drag'),
        html.Br(),
...
def display_page(pathname):
    ...
    if '/graph/' in pathname:
        return html.Div([
            graph_page_layout(pathname.split("/")[-1])
        ])
    else:
        return '404'
...
def update_datatable(episode, graph_name):
    ...
    return dash_table.DataTable(df.to_dict('records'), [{'id': x, 'name': x} for x in df.columns])
