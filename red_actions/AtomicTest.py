...
class Dependency():
    ...
    def __init__(self, executor_name : str = "", description : str = "", prerequisite_command : str = "", get_prerequisite_command : str = ""):
...
class AtomicTest():
    ...
    def __init__(self, atomic_test_dict):
        ...
        self.auto_generated_guid = atomic_test_dict["auto_generated_guid"] if "auto_generated_guid" in valid_params else ""
        self.description = atomic_test_dict["description"] if "description" in valid_params else ""
        self.supported_platforms = atomic_test_dict["supported_platforms"] if "supported_platforms" in valid_params else []
        ...
        if "executor" in valid_params:
            ...
            executor_command = executor["command"] if "command" in list(executor.keys()) else ""
            executor_cleanup_command = executor["cleanup_command"] if "cleanup_command" in list(executor.keys()) else ""
            executor_elevation_required = executor["elevation_required"] if "elevation_required" in list(executor.keys()) else True # TODO: Is it better for default elevation required to be T or F???
            
            self.executor = Executor(name=executor_name, command=executor_command, cleanup_command=executor_cleanup_command, elevation_required=executor_elevation_required)
        else:
            self.executor = None
        ...
        if "input_arguments" in valid_params:
            ...
            for name in list(input_arguments.keys()):
                ...
                inargs.append(InputArgument(name, arg["description"], arg["type"], arg["default"]))
        ...
        if "dependencies" in valid_params:
            ...
            for d in dependencies:
                self.dependencies.append(Dependency(self.dependency_executor_name, d["description"], d["prereq_command"], d["get_prereq_command"]))
        else:
            self.dependencies = []
